...
class SmartManeuverController : public ManeuverControllerBase // Modify stuff here!
{
...
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        static float prev_fwd_vel = 0;
        static float prev_turn_vel = 0;
        float alpha_smooth = 0.1; // Smoothing factor, adjust as needed

        float fwd_vel = vel_sign * pid[0] * d_fwd;
        float turn_vel = pid[1] * alpha + pid[2] * beta;

        // // Smooth the velocities
        // if (fwd_vel - prev_fwd_vel > 0.01)
        //     fwd_vel = prev_fwd_vel + 0.01;
        // else if (fwd_vel - prev_fwd_vel < -0.01)
        //     fwd_vel = prev_fwd_vel - 0.01;
        // else
        //     fwd_vel = alpha_smooth * fwd_vel + (1 - alpha_smooth) * prev_fwd_vel;
        // // fwd_vel = 0.6;
        // if (turn_vel - prev_turn_vel > 0.01)
        //     turn_vel = prev_turn_vel + 0.01;
        // else if (turn_vel - prev_turn_vel < -0.01)
        //     turn_vel = prev_turn_vel - 0.01;
        // else
        //     turn_vel = alpha_smooth * turn_vel + (1 - alpha_smooth) * prev_turn_vel;

        // // Update previous velocities
        // prev_fwd_vel = fwd_vel;
        // prev_turn_vel = turn_vel;
...
class CarrotManeuverController : public ManeuverControllerBase
{
private:
    float lookahead_distance = 1.0; // Distance to the "carrot"
    float pid[3] = {4.0, 5.5, 0.0}; //kp, ka, kb
    float d_end_crit = 0.05;
    // float d_end_midsteps = 0.1;     // This is used for SpeedRun Only
    float d_end_midsteps = 0.08;   // This is used for Exploration

public:
    CarrotManeuverController() = default;
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        float carrot_x = target.x;
        float carrot_y = target.y;

        if (distance_to_target > lookahead_distance) {
            carrot_x = pose.x + lookahead_distance * (dx / distance_to_target);
            carrot_y = pose.y + lookahead_distance * (dy / distance_to_target);
        }
        // else if (distance_to_target < lookahead_distance) {
        //     carrot_x = target.x + (lookahead_distance - distance_to_target) * std::cos(target.theta);
        //     carrot_y = target.y + (lookahead_distance - distance_to_target) * std::sin(target.theta);
        // }

        // Calculate the distance and angle to the carrot point
        float d_fwd = sqrt(pow(carrot_x - pose.x, 2) + pow(carrot_y - pose.y, 2));
        float angle_to_carrot = atan2(carrot_y - pose.y, carrot_x - pose.x);
        float alpha = angle_diff(angle_to_carrot, pose.theta);

        // Calculate the forward and turn velocities
        float beta = wrap_to_pi(target.theta - (alpha + pose.theta));
        float fwd_vel = pid[0] * d_fwd;
        if (fwd_vel < 0.4)
            fwd_vel = 0.4;
        float turn_vel = pid[1] * alpha + pid[2] * beta;

        // If alpha is more than 45 degrees, turn in place and then go
        if (fabs(alpha) > M_PI/3)
        {
            fwd_vel = 0;
        }
    ...
    bool target_reached(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target, bool is_end_pose) override
    {
        float distance = d_end_midsteps;
        if (is_end_pose)
            distance = d_end_crit;
            ...
            { 
                if(turn_controller.target_reached_final_turn(pose, target))
                {
		            if(!assignNextTarget())
                    {
                        ...
                        // Used for exploration, comment out for SpeedRun
                        // cmd = {0, 0, 0, M_PI_4};
                        // sleep(15);
                    }
                } 
...
private:
    ...
    CarrotManeuverController carrot_controller;
