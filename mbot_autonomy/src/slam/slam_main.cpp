...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
        ...
        if (ctrl_c_pressed) 
        {
            // XX: Save the current pose to a file.
            mbot_lcm_msgs::pose2D_t start_pose;
            start_pose = slam->getCurrentPose();
            std::ofstream poseFile("/home/mbot/mbot_ws/mbot_autonomy/src/slam/current_pose.txt");
            if (poseFile.is_open())
            {
                poseFile << start_pose.x << " " << start_pose.y <<  " " <<start_pose.theta << std::endl;
                poseFile.close();
                std::cout << "start_pose_y is:" << start_pose.y << std::endl;
                std::cout << LOG_HEADER << "Saved current pose to current_pose.txt" << std::endl;
            }
            else
            {
                std::cout << LOG_HEADER << "Unable to open file to save pose." << std::endl;
            }
            break;
        }
