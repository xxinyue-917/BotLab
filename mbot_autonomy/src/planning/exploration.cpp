...
int8_t Exploration::executeInitializing(void)
{
    ...
    frontier_processing_t result = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);
    currentPath_ = result.path_selected;
...
int8_t Exploration::executeExploringMap(bool initialize)
{
   ...
   // Create the status message
    ...
    vel_limits_.vx = 0.2;
    vel_limits_.vy = 0;
    vel_limits_.wz = M_PI / 2.0;
    lcmInstance_->publish(MBOT_MAX_VEL_CHANNEL, &vel_limits_);

    /// TODO: Implement logic for finding and selecting 
    //        the next frontier to explore and planning the path to it.
    frontiers_ = find_map_frontiers(currentMap_, currentPose_, minFrontierLength_);
    if (frontiers_.empty())
    {
        // Exporation has finished
        // mbot_lcm_msgs::twist2D_t rot_;
        // rot_.vx = 0;
        // rot_.vy = 0;
        // rot_.wz = M_PI / 2.0;
        // lcmInstance_->publish(MBOT_MOTOR_COMMAND_CHANNEL, &rot_);
        sleep(10);
        // mbot_lcm_msgs::twist2D_t zero_;
        // zero_.vx = 0; 
        // zero_.vy = 0;
        // zero_.wz = 0;
        // lcmInstance_->publish(MBOT_MOTOR_COMMAND_CHANNEL, &zero_);
        status.status = mbot_lcm_msgs::exploration_status_t::STATUS_COMPLETE;
    ...
    else
    {
        ...
        mbot_lcm_msgs::path2D_t previousPath = currentPath_;
        goal_pose.x = previousPath.path.back().x;
        goal_pose.y = previousPath.path.back().y;
        if (fabs(goal_pose.x - currentPose_.x) < kReachedPositionThreshold && fabs(goal_pose.y - currentPose_.y) < kReachedPositionThreshold)
        {
            // Select next path
            // mbot_lcm_msgs::twist2D_t rot_;
            // rot_.vx = 0;
        ...
        // Check if there's a valid path to frontier
        ...
        {
            // Failed
            status.status = mbot_lcm_msgs::exploration_status_t::STATE_RETURNING_HOME;
        }
    }
...
int8_t Exploration::executeReturningHome(bool initialize)
{
    ...
    */
    ...
    // Set the home pose as the origin as exploration may need to restart
    homePose_.x = 0.0;
    homePose_.y = 0.0;
    homePose_.theta = 0.0;
